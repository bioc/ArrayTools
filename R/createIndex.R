...
function(..., mydir=getwd(), index.file= "index.html", 
    ...
    if (is.element(arrayType, c("hugene10sttranscriptcluster", "mogene10sttranscriptcluster"))) {  ## For genechip array only
    ...
    if (file.exists("Normalized_Data")) {    
        HTML("<hr /> <p align=left><font size='5', face='Arial'> Data Preprocessing", 
            file = index.file)
        HTML("<ul>", file=index.file)     
        if (is.element(arrayType, c("hugene10sttranscriptcluster", "mogene10sttranscriptcluster"))){            
